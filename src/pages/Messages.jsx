...
const MessageIndicator = ({textCount}) => (
    <span 
        className={`bg-white-800 text-sm font-bold ${textCount>0? 'visible' : 'invisible'} border-2 ${(textCount>250 && textCount<350) ? 'border-yellow-600 text-yellow-500':textCount>=350 ? 'border-red-700 text-red-600' : 'border-indigo-800 text-indigo-900'} flex items-center justify-center self-end mr-2 my-2 rounded-full w-8 h-8`}>
            {textCount}
    </span>
)
